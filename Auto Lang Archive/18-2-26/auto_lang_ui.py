...
class SettingsWindow:
    ...
    def _add_app(self):
        ...
        for item in self.app_tree.get_children():
            if self.app_tree.item(item)['values'][0] == exe:
                ...
                return
        self.app_tree.insert('', 'end', values=(exe, _lang_display(lang)))
        self.app_exe_var.set('')
    ...
    def _add_chat(self):
        ...
        if not exe or not title:
            return
        self.chat_tree.insert('', 'end', values=(exe, title, _lang_display(lang)))
    ...
    def _build_browser_defaults_tab(self, notebook):
        frame = ttk.Frame(notebook, padding=12)
        notebook.add(frame, text='🌐 אתרים')

        ttk.Label(frame, text='שפת ברירת מחדל לפי אתר / דף', style='Header.TLabel').pack(anchor='e', pady=(0, 6))
        ttk.Label(frame, text=(
            'הגדר שפה לפי מילת מפתח בכותרת הדף בדפדפן.\n'
            'עובד עם כל דפדפן (Chrome, Edge, Firefox ועוד).\n'
            'למשל: "YouTube" → עברית, "GitHub" → English'
        ), style='Subtitle.TLabel', justify='right').pack(anchor='e')
    ...
    def _add_browser_rule(self):
        ...
        if not keyword:
            return
        self.browser_tree.insert('', 'end', values=(keyword, _lang_display(lang)))
