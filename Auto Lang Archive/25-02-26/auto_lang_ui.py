...
from PySide6.QtGui import (
    ...
    QFontMetrics, QPainterPath, QGuiApplication,
)
...
class FloatingWidget(QWidget):
    ...
    def _paint_text_words(self, p):
        ...
        if is_rtl:
            ...
            for word in words1:
                ...
                p.setPen(QColor('#fbbf24' if is_corr else COLORS['text_primary']))
    ...
    def _create_editor_widgets(self):
        ...
        """)
        layout.addWidget(self._trans_widget, 1)
    ...
    def _translate_worker(self, text: str):
        try:
            ...
            if result is None:
                QTimer.singleShot(0, lambda: self._show_translation_result(
                    '\u274c \u05ea\u05e8\u05d2\u05d5\u05dd \u05e0\u05db\u05e9\u05dc', error=True))
            else:
                source = 'Google' if translator._online else 'Argos'
                QTimer.singleShot(0, lambda r=result, s=source:
                                  self._show_translation_result(r, source=s))
        except Exception as e:
            QTimer.singleShot(0, lambda: self._show_translation_result(
                f'\u05e9\u05d2\u05d9\u05d0\u05d4: {e}', error=True))
    ...
    def _on_right_click_global(self):
        ...
        QTimer.singleShot(0, lambda: self._show_translate_popup(x, y))
    ...
    def _on_popup_translate_click(self):
        ...
        def _do_copy():
            ...
            if text and text.strip():
                ...
                QTimer.singleShot(0, self._fill_and_translate)

        self._pending_selection = ''
        threading.Thread(target=_do_copy, daemon=True).start()

    def _fill_and_translate(self):
        selected = getattr(self, '_pending_selection', '')
        self._pending_selection = ''
        if not selected:
            return
        ...
        if self._text_widget:
            self._text_widget.setPlainText(selected)
