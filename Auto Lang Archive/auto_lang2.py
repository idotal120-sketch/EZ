...
# Low-level hook API signatures
user32.SetWindowsHookExW.argtypes = [ctypes.c_int, ctypes.c_void_p, wintypes.HINSTANCE, wintypes.DWORD]
user32.SetWindowsHookExW.restype = ctypes.c_void_p
user32.UnhookWindowsHookEx.argtypes = [ctypes.c_void_p]
user32.UnhookWindowsHookEx.restype = wintypes.BOOL
user32.CallNextHookEx.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_ulonglong, ctypes.c_ulonglong]
user32.CallNextHookEx.restype = ctypes.c_long

# Message pump API signatures
    ...
    while not stop_event.is_set():
        try:
            ...
            if focus_changed or title_changed:
                ...
                # Clear the floating panel on window / chat switch
                global _prev_words
                _prev_words = []
...
def _on_mouse_click():
    ...
    if not injecting.is_set():
        with state.lock:
            if state.buffer:
                _dbg(f'[MOUSE] Click detected, resetting buffer ({state.buffer!r})')
                state.buffer = ''
                state.pending = []
...
def main():
    ...
    # Install low-level mouse hook (reset buffer on click)
    _dbg('[MAIN] About to install mouse hook...')
    _install_mouse_hook()
    _dbg('[MAIN] Mouse hook thread launched')
